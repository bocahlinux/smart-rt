...
- Primary key: `id` (AutoField / BigAutoField)
...
| role | VARCHAR(20) | NOT NULL, DEFAULT 'warga' | admin/pengurus/warga |
...
| user_id | INT | FK → users.id, UNIQUE | — | 1:1 dengan user |
...
|--------|------|-------------|----------------|-------------|
| id | INT | PK, AUTO_INCREMENT | — | |
| kategori_id | INT | FK → kategori_transaksi.id | — | |
...
|--------|------|-------------|----------------|-------------|
| id | INT | PK, AUTO_INCREMENT | — | |
| warga_id | INT | FK → warga_profiles.id | — | |
...
- **Primary keys**: AutoField (auto-increment)
